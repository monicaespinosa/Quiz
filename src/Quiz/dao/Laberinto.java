...
public class Laberinto extends Juego implements Runnable{
    ...
    public void colision(){
    	...
    	for(int i = 0; i<getMundos().getObjetos().size(); i++){
    		for(int j = 0; j<getMundos().getObjetos().get(i).getCasillas().size(); i ++){
    			int objective_x = getMundos().getObjetos().get(i).getCasillas().get(j).getnPosicion_x();
        		int objective_y = getMundos().getObjetos().get(i).getCasillas().get(j).getnPosicion_y();
        		if(hero_x == objective_x && hero_y == objective_y){
        			getMundos().getObjetos().remove(i);
        		}
    		}
    	}
    	...
    	for(int i = 0; i <getMundos().getObstaculos().size(); i++){
